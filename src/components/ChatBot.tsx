    ...
    <>
      {!open && (
        <button
          ...
          className="fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-4 sm:bottom-6 sm:right-6 z-[120] rounded-full bg-gradient-to-r from-primary-500 to-primary-400 hover:from-primary-600 hover:to-primary-500 text-white p-3 sm:p-4 shadow-lg ring-2 ring-primary-500/30 pointer-events-auto"
        >
          <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6" />
        </button>
      )}
      {open && (
        <div className="fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-4 sm:bottom-6 sm:right-6 z-[120] w-[min(92vw,380px)] bg-dark-800/95 backdrop-blur border border-gray-700 rounded-2xl shadow-2xl overflow-hidden ring-1 ring-black/30 pointer-events-auto">
